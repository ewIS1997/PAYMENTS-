      ...
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 animate-pulse">
          <div className="flex items-start gap-3">
            <div className="w-6 h-6 bg-gray-200 dark:bg-gray-700 rounded mt-1"></div>
            <div className="flex-1 space-y-2">
              <div className="h-6 bg-gray-200 dark:bg-gray-700 rounded w-2/3"></div>
              <div className="h-5 bg-gray-100 dark:bg-gray-600 rounded w-1/3"></div>
              <div className="h-5 bg-gray-100 dark:bg-gray-600 rounded w-1/4"></div>
            </div>
          </div>
        </div>
      ))}
...
export function SkeletonCards({ count = 3 }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-gray-100 dark:bg-gray-800 rounded-xl h-28 animate-pulse"></div>
      ))}
    </div>
  );
}
